...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `مقالات نوشته شده توسط ${author.name} در وبلاگ High Win Rate`;
  const url = `https://highwinrate.com/blog/author/${slug}`;
  const authorImage = author.image ? getImageUrl(author.image) : undefined;
  ...
  return {
    ...
    openGraph: {
      ...
      images: authorImage ? [{ url: authorImage, width: 400, height: 400, alt: author.name }] : [],
    },
    twitter: {
      card: 'summary',
      title,
...
export default async function AuthorPage({ params }: Props) {
  ...
  const personSchema = {
    ...
    '@type': 'Person',
    name: author.name,
    url: `https://highwinrate.com/blog/author/${slug}`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: author.name,
        item: `https://highwinrate.com/blog/author/${slug}`,
      },
    ],
  };
